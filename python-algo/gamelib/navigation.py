...
class ShortestPathFinder:
    ...
    def initialize_map(self, game_state):
        ...
        self.game_map = [
            [Node() for x in range(self.game_state.ARENA_SIZE)]
    ...
    def navigate_multiple_endpoints(self, start_point, end_points, game_state):
        ...
        debug_write(f"Trying to path from {start_point} to any of {end_points}")
        if game_state.contains_stationary_unit(start_point):
            debug_write(f"Start point {start_point} is blocked by a unit.")
            return
    ...
    def _idealness_search(self, start, end_points):
        ...
        while not current.empty():
            ...
            for neighbor in self._get_neighbors(search_location):
                if (
                    not self.game_state.game_map.in_arena_bounds(neighbor)
                ...
                if (
                    not self.game_map[x][y].visited_idealness
                    and not self.game_map[x][y].blocked
                ):
    ...
    def _choose_next_move(self, current_point, previous_move_direction, end_points):
        ...
        for neighbor in neighbors:
            ...
            if not new_best and not self._better_direction(
                current_point,
                neighbor,
                ideal_neighbor,
    ...
    def _better_direction(
        self, prev_tile, new_tile, prev_best, previous_move_direction, end_points
    ):
        ...
        if (
            previous_move_direction == self.HORIZONTAL
            and not new_tile[0] == prev_best[0]
        ):
        ...
        if new_tile[1] == prev_best[1]:  # If they both moved horizontal...
            if (
                direction[0] == 1 and new_tile[0] > prev_best[0]
            ):  # If we moved right and right is our direction, we moved towards our direction
                return True
            if (
                direction[0] == -1 and new_tile[0] < prev_best[0]
            ):  # If we moved left and left is our direction, we moved towards our direction
                return True
            return False
        if new_tile[0] == prev_best[0]:  # If they both moved vertical...
            if (
                direction[1] == 1 and new_tile[1] > prev_best[1]
            ):  # If we moved up and up is our direction, we moved towards our direction
                return True
            if (
                direction[1] == -1 and new_tile[1] < prev_best[1]
            ):  # If we moved down and down is our direction, we moved towards our direction
                return True
            return False
        return True
    ...
    def print_map(self):
        ...
        if not self.initialized:
            debug_write(
                "Attempted to print_map before pathfinder initialization. Use 'this_object.initialize_map(game_state)' to initialize the map first"
            )
            return
